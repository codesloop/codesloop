...
#if (AES_IMPLEMENTAION == 3) /* Gladman */
#include "aes.h"
typedef aes_encrypt_ctx    aes_int_key[1]; /* AES internal */
#define aes_encryption(in,out,int_key) \
	    aes_encrypt((unsigned char *)(in),(unsigned char *)(out),(aes_encrypt_ctx *)(int_key))
#define aes_key_setup(key,int_key) \
	    aes_encrypt_key128((key),(int_key))
#elif  (AES_IMPLEMENTAION == 2) /* Barreto */
...
#define aes_key_setup(key,int_key) \
	    ...
	    UMAC_KEY_LEN*8)
#elif (AES_IMPLEMENTAION == 1) /* OpenSSL */
...
#else
#error !!! No AES_IMPLEMENTAION defined !!!
#endif
...
{
	ctx->umac_ctx = umac_new(user_key);
	ctx->lengths[0] = 0;
	ctx->lengths[1] = 0;
	ctx->lengths[2] = 0;
}
...
static void umac_ae_add(char *d,
         unsigned int dbytes,
         umac_ae_ctx_t *ctx)
{
	unsigned initial, remaining, zeros;
	char leftover[32];
	
	zeros = (32 - (dbytes % 32)) % 32; /* zeroes needed after d */
	
	/* Deal with any multiple of 32 bytes */
	remaining = dbytes % 32;
	initial   = dbytes & ~(32-1);
	if (initial)
		umac_update(ctx->umac_ctx, d, initial);
	
	/* Finally, if any bytes remain, pad to 32 bytes */
	if (remaining) {
		memcpy(leftover, d+initial, remaining);
		memset(leftover+remaining, 0, zeros);
		umac_update(ctx->umac_ctx, leftover, 32);
	}
}
...
void umac_ae_header(char *h,
         ...
         umac_ae_ctx_t *ctx)
{
	ctx->lengths[0] = hbytes;
	umac_ae_add(h, hbytes, ctx);
}
...
void umac_ae_footer(char *f,
         ...
         umac_ae_ctx_t *ctx)
{
	ctx->lengths[2] = fbytes;
	umac_ae_add(f, fbytes, ctx);
}
		 ...
		 umac_ae_ctx_t *ctx)
{
	...
	uint64_t      *itp        = (uint64_t *)it,
	              *otp        = (uint64_t *)ot;
	aes_int_key   *cipher_key  = &(ctx->umac_ctx->pdf.prf_key);
	
	iters     = tbytes / 16;
	remaining = tbytes % 16;
	
	/* Copy supplied nonce to ctx and locally */
	((uint64_t *)nonce)[0] = *(uint64_t *)(ctx->nonce) = *(uint64_t *)n;
	((uint64_t *)nonce)[1] = 0; 
	
	/* Store length of encryption */
	ctx->lengths[1] = tbytes;
	
	for (ctr = 1; ctr <= iters; ctr++) {
		STORE_UINT32_BIG(nonce+3, ctr);
		aes_encryption(nonce, tmp, cipher_key);
		otp[0] = itp[0] ^ tmp[0];
		otp[1] = itp[1] ^ tmp[1];
		itp += 2;
		otp += 2;
...
void umac_ae_finalize(char *tag, umac_ae_ctx_t *ctx)
{
	...
	bits = ctx->lengths[0] * UINT64_C(8);
	STORE_UINT32_BIG( buf+0, (uint32_t)(bits >> 32));
	STORE_UINT32_BIG( buf+1, (uint32_t)(bits)      );
	bits = ctx->lengths[1] * UINT64_C(8);
	STORE_UINT32_BIG( buf+2, (uint32_t)(bits >> 32));
	STORE_UINT32_BIG( buf+3, (uint32_t)(bits)      );
	bits = ctx->lengths[2] * UINT64_C(8);
	STORE_UINT32_BIG( buf+4, (uint32_t)(bits >> 32));
	STORE_UINT32_BIG( buf+5, (uint32_t)(bits)      );
	buf[6] = 0;
	buf[7] = 0;

	umac_update(ctx->umac_ctx, (char *)buf, 32);
	umac_final(ctx->umac_ctx, tag, ctx->nonce);
	ctx->lengths[0] = 0;
	ctx->lengths[1] = 0;
	ctx->lengths[2] = 0;
}
...
int ae_verify(void)
{
	...
	#if (UMAC_OUTPUT_LEN == 8)
	char *should_be[] = {"0000000000000000","0000000000000000",
	                     "0000000000000000","0000000000000000",
	                     "0000000000000000"};
	#elif (UMAC_OUTPUT_LEN == 16)
	char *should_be[] = {"00000000000000000000000000000000",
	...
	p = malloc(sizeof(umac_ae_ctx_t) + 16);
	ctx = (umac_ae_ctx_t *)(((size_t)p + 16) & ~((size_t)15));
	p = malloc(buf_len + 32);
	m = ( char *)(((size_t)p + 16) & ~((size_t)15));
	p = malloc(buf_len + 32);
	ct = ( char *)(((size_t)p + 16) & ~((size_t)15));
	p = malloc(buf_len + 32);
	pt = ( char *)(((size_t)p + 16) & ~((size_t)15));
	...
	for (i = 0; i < sizeof(vector_lengths)/sizeof(unsigned int); i++) {
		...
		umac_ae_header(m, vector_lengths[i], ctx);
		umac_ae_encrypt(m, ct, vector_lengths[i], nonce, ctx);
		umac_ae_footer(m, vector_lengths[i], ctx);
		umac_ae_finalize(tag, ctx);
		umac_ae_header(m, vector_lengths[i], ctx);
		umac_ae_decrypt(ct, pt, vector_lengths[i], nonce, ctx);
		umac_ae_footer(m, vector_lengths[i], ctx);
		umac_ae_finalize(tag2, ctx);
		printf("%u bytes (\'abc\' * %u):\n", vector_lengths[i], vector_lengths[i]/3);
		printf("Encrypt/decrypt %s, tags %s\n",
		      (memcmp(pt, m, vector_lengths[i])  ? "mismatch" : "match"),
		      (memcmp(tag, tag2, UMAC_OUTPUT_LEN) ? "mismatch" : "match"));
		#if (UMAC_OUTPUT_LEN == 8)
		printf("Should be: %s\n",should_be[i]);
		pbuf(tag, UMAC_OUTPUT_LEN, "Is       ");
		#elif (UMAC_OUTPUT_LEN == 16)
	...
	for (i = 0; i < sizeof(speed_lengths)/sizeof(unsigned int); i++) {
		...
		for (j = 0; j < speed_iters[i]; j++) {
			umac_ae_encrypt(m, ct, speed_lengths[i], nonce, ctx);
			umac_ae_finalize(tag, ctx);
			nonce[7]++;
		}
		ticks = clock() - ticks;
...
static void speed_test(void)
{
    ...
    if (length_range_low < length_range_high) {
        for (i = length_range_low; i <= length_range_high; i++) {
            cpb = run_cpb_test(ctx, i, data_ptr, data_len);
